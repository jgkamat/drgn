...
static PyObject *DrgnType_get_template_parameters(DrgnType *self)
{
	...
	for (i = 0; i < num_templates; i++) {
		...
		Py_INCREF(self);
		item->lazy_type.state = DRGNPY_LAZY_TYPE_UNEVALUATED;
		item->lazy_type.lazy_type = &template->type;
		if (template->name) {
				  ...
				  PyObject *kwds)
{
	...
	template->lazy_type.state = type_state;
	Py_INCREF(type_arg);
	template->lazy_type.obj = type_arg;
	Py_INCREF(name);
	template->name = name;
	return template;
}
...
static PyGetSetDef TypeTemplateParameter_getset[] = {
	{"type", (getter)LazyType_get, NULL, drgn_TypeTemplateParameter_type_DOC, NULL},
	{},
};
			   ...
			   PyObject *item, bool *can_cache)
{
	...
	if (lazy_type_from_py(&template_type, (LazyType *)template,
			      prog, can_cache) == -1)
		return -1;
	struct drgn_error *err =
...
DrgnType *Program_function_type(Program *self, PyObject *args, PyObject *kwds)
{
	...
	if (templates_obj != Py_None) {
		...
		PyObject *cached_templates = PySequence_Tuple(templates_obj);
		if (!cached_templates)
			goto err_builder;
