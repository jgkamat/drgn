...
static struct drgn_error *read_cu(struct drgn_dwarf_index_cu_buffer *buffer)
{
	...
	/* Skip type_signature and type_offset if present
	   to properly place buffer pos */
	if (buffer->cu->is_type_unit) {
		if ((err = binary_buffer_skip(&buffer->bb, buffer->cu->is_64_bit ? 16 : 12))) {
			return err;
		}
	}
		    ...
		    struct drgn_dwarf_index_cu_buffer *buffer)
{
	...
	Elf_Data *debug_info = cu->module->scns[
				  ...
				  struct drgn_debug_info_module *module)
{
	...
	if (module->scns[DRGN_SCN_DEBUG_TYPES])
...
drgn_dwarf_index_update_end(struct drgn_dwarf_index_update_state *state)
{
	...
	for (size_t i = state->old_cus_size; i < dindex->cus.size; i++) {
		...
		buffer.bb.pos += index_cu_header_offset(cu);
					    ...
					    Dwarf_Die *die_ret)
{
	...
	uint64_t size = die->module->scns[DRGN_SCN_DEBUG_INFO]->d_size;
	if (die->addr >= start &&
	    die->addr < start + size) {
		if (!dwarf_offdie(dwarf, die->addr - start, die_ret))
			return drgn_error_libdw();
	} else {
		start = (uintptr_t)die->module->scns[DRGN_SCN_DEBUG_TYPES]->d_buf;
		/* Assume .debug_types */
