...
static struct drgn_error *translate_form(
	uint64_t form,
	uint8_t indirect_section,
	struct drgn_dwarf_index_cu *cu,
	struct binary_buffer *bb,
	uint8_t *insn_ret,
	uint8_t *die_flags_ret)
{
	...
	case ATTRIB_DECLARATION_INDIRECT:
		...
		case DW_FORM_flag_present:
			 ...
			 * use it.
			 */
			*die_flags_ret |= DIE_FLAG_DECLARATION;
			return NULL;
		case DW_FORM_indirect:
...
static struct drgn_error *read_abbrev_decl(struct drgn_debug_info_buffer *buffer,
					   struct drgn_dwarf_index_cu *cu,
					   ...
					   struct uint8_vector *insns)
{
	...
	for (;;) {
		...
		if (name == DW_AT_sibling) {
			if ((err = translate_form(form, ATTRIB_SIBLING_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		} else if (name == DW_AT_name && should_index) {
			if ((err = translate_form(form, ATTRIB_NAME_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		} else if (name == DW_AT_stmt_list && cu->module->scns[DRGN_SCN_DEBUG_LINE]) {
			if ((err = translate_form(form, ATTRIB_STMT_LIST_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		} else if (name == DW_AT_decl_file && should_index &&
			   /* Namespaces are merged, so we ignore their file. */
			   tag != DW_TAG_namespace) {
			if ((err = translate_form(form, ATTRIB_DECL_FILE_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		} else if (name == DW_AT_declaration && should_index) {
			if ((err = translate_form(form, ATTRIB_DECLARATION_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		} else if (name == DW_AT_specification && should_index) {
			if ((err = translate_form(form, ATTRIB_SPECIFICATION_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
				return err;
		}
		if (insn != 0)
			goto append_insn;
		...
		if (form == DW_FORM_flag_present)
			continue;
		if ((err = translate_form(form, ATTRIB_INDIRECT, cu, &buffer->bb, &insn, &die_flags)))
			return err;
		if (insn != 0 && insn > INSN_MAX_SKIP)
			goto append_insn;
...
index_specification(struct drgn_dwarf_index *dindex, uintptr_t declaration,
		    struct drgn_debug_info_module *module, size_t offset)
{
...
specification_ref_addr:
			...
			case ATTRIB_SPECIFICATION_INDIRECT:
				if ((err = binary_buffer_next_uleb128(&buffer->bb, &code)))
					return err;
				uint8_t tmp_insn = 0;
				if ((err = translate_form(code, insn, cu, &buffer->bb, &tmp_insn, &added_flags)))
					return err;
				...
				if (tmp_insn == 0)
				{
					if (code == DW_FORM_flag_present)
						continue;
					if ((err = translate_form(code, ATTRIB_INDIRECT, cu, &buffer->bb, &tmp_insn, &added_flags)))
						return err;
				}
				...
				if (tmp_insn != 0) {
					...
					// Skip of 0, should not be possible
					continue;
			default:
				skip = insn;
skip:
