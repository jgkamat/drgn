...
LIBDRGN_PUBLIC struct drgn_error *
drgn_template_parameter_type(struct drgn_template_parameter *template_parameter,
			     struct drgn_qualified_type *ret)
{
	return drgn_lazy_parameter_get_type(&template_parameter->parameter, ret);
}
...
drgn_type_builder_add_template_parameter(struct drgn_program *prog,
					 ...
					 const char *name)
{
	struct drgn_error *err = drgn_lazy_parameter_check_prog(&parameter, prog);
	if (err)
		return err;
			  ...
			  struct drgn_type **ret)
{
	...
	if (!is_complete && size != 0)
		return drgn_error_create(DRGN_ERROR_INVALID_ARGUMENT,
					 "Incomplete types cannot have a size.");
	if (!is_complete && builder->members.size != 0)
		return drgn_error_create(DRGN_ERROR_INVALID_ARGUMENT,
					 "Incomplete types cannot have any members.");
	...
	int *depth, bool *ret)
{
	...
	for (i = 0; i < num_a; i++) {
		...
		if ((templates_a[i].name && templates_b[i].name) &&
		    (!templates_a[i].name || !templates_b[i].name ||
	...
	for (size_t i = 0; i < prog->created_types.size; i++) {
		...
		if (drgn_type_has_template_parameters(type)) {
			...
			for (size_t i = 0; i < num_templates; i++)
				drgn_lazy_parameter_deinit(&templates[i].parameter);
			free(templates);
		}
		free(type);
	}
