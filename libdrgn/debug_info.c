...
{
	return drgn_type_from_dwarf_internal(dbinfo, die, bias, true, NULL, ret);
}
...
static struct drgn_error *
parse_member(struct drgn_debug_info *dbinfo, Dwarf_Die *die,
	     uint64_t bias,
					 ...
					 struct drgn_template_parameter_vector *templates)
{
	...
	if (attr) {
		...
		if (!name) {
			return drgn_error_create(DRGN_ERROR_OTHER,
						 "DW_TAG_template_type_parameter has invalid DW_AT_name");
		}
	} else {
		name = NULL;
	}

	err = drgn_lazy_type_from_dwarf(dbinfo, die, false,
					"DW_TAG_template_type_parameter has invalid DW_AT_name",
					tag_name,
					&template_parameter);
	if (err)
			      ...
			      enum drgn_type_kind kind, struct drgn_type **ret)
{
	...
	while (r == 0) {
		if ((!declaration && dwarf_tag(&child) == DW_TAG_member) ||
			...
			member = child;
		} else if (dwarf_tag(&child) == DW_TAG_template_type_parameter) {
			     ...
			     struct drgn_type **ret)
{
	...
	struct drgn_error *err = drgn_type_from_dwarf_child(dbinfo, die, bias,
			   ...
			   struct drgn_type **ret)
{
	...
	err = drgn_type_from_dwarf_child(dbinfo, die, bias,
					 drgn_language_or_default(lang),
...
parse_formal_parameter(struct drgn_debug_info *dbinfo, Dwarf_Die *die,
		       uint64_t bias, struct drgn_function_type_builder *builder)
{
	Dwarf_Attribute attr_mem, *attr;
	const char *name;
	if ((attr = dwarf_attr_integrate(die, DW_AT_name, &attr_mem))) {
			      ...
			      struct drgn_type **ret)
{
	...
	while (r == 0) {
		...
		case DW_TAG_formal_parameter:
			...
			err = parse_formal_parameter(dbinfo, &child, bias, &builder);
			if (err)
				goto err;
			break;
			      ...
			      struct drgn_qualified_type *ret)
{
	...
	case DW_TAG_base_type:
		err = drgn_base_type_from_dwarf(dbinfo, die, bias, lang, &ret->type);
		break;
	...
	case DW_TAG_enumeration_type:
		err = drgn_enum_type_from_dwarf(dbinfo, die, bias, lang, &ret->type);
		break;
	case DW_TAG_typedef:
...
{
	struct drgn_qualified_type qualified_type;
	struct drgn_error *err = drgn_type_from_dwarf_child(dbinfo, die, bias, NULL,
							    "DW_TAG_variable",
							    true, true, NULL,
							    &qualified_type);
	if (err)
...
drgn_debug_info_find_object_in_type_die(struct drgn_dwarf_index_die *index_die,
	...
	if ((err = drgn_dwarf_index_get_die(index_die, &die, &bias)))
		return err;
	if ((err = drgn_type_from_dwarf(dbinfo, &die, bias, &type)))
		return err;

	if (!drgn_type_has_members(type.type)) {
		return drgn_error_format(DRGN_ERROR_OTHER,
					 "Internal Error parsing :: on tag: %s",
					 index_die->tag);
	}
	...
	for (int i = 0; i < drgn_type_num_members(type.type); i++) {
		if (strcmp(members[i].name, name) == 0 && members[i].parameter.is_object) {
			// We found our member (hopefully)
			struct drgn_object obj;
			err = drgn_lazy_parameter_get_object(&members[i].parameter, &obj);
			if (err)
				return err;
			drgn_object_init(ret, drgn_object_program(&obj));
			drgn_object_copy(ret, &obj);
			*ret = obj;
			return NULL;
		}
	}
